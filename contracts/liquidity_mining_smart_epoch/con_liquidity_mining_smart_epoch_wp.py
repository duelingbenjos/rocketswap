...
LIQUIDITY_TOKEN = "con_uwarriors_lst001"
YIELD_TOKEN = con_uwarriors_lst001
...
def seed():
    ...
    EpochMaxRatioIncrease.set(50)
    EpochMinTime.set(3600)
    TrustedImporters.set([])

    Epochs[0] = {"time": now, "staked": 0, "amt_per_hr": 384}
    EmissionRatePerHour.set(384) # 11507 / 24 * 0.8

    meta["version"] = "0.5"
    meta["type"] = "liquidity_mining_smart_epoch"
    meta["STAKING_TOKEN"] = "con_uwarriors_lst001"
    meta["YIELD_TOKEN"] = "con_uwarriors_lst001"

    DevRewardPct.set(1 / 20)

    StartTime.set(datetime.datetime(year=2021, month=7, day=29, hour=22))
    EndTime.set(datetime.datetime(year=2022, month=7, day=29, hour=22))

    OpenForBusiness.set(False)
