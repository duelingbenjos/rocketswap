...
# Reward Token
import con_marmite100_contract
...
LIQUIDITY_TOKEN = "con_marmite100_contract"  # TAU/RSWP Pair
YIELD_TOKEN = con_marmite100_contract
...
def seed():
    ...
    DevRewardWallet.set("000000")
    CurrentEpochIndex.set(0)
    StakedBalance.set(0)
    WithdrawnBalance.set(0)
    EpochMaxRatioIncrease.set(10)
    EpochMinTime.set(86400)
    TrustedImporters.set([])

    Epochs[0] = {"time": now, "staked": 0, "amt_per_hr": 3333333} # 1bn over 8 months

    meta["version"] = "0.4"
    meta["type"] = "liquidity_mining_smart_epoch"  # staking || lp_farming
    meta["STAKING_TOKEN"] = "con_marmite100_contract"
    meta["YIELD_TOKEN"] = "con_marmite100_contract"

    EmissionRatePerHour.set(3333333)  # 1200000 RSWP per year = 10% of supply
    DevRewardPct.set(1/10)

    # The datetime from which you want to allow staking.
    StartTime.set(datetime.datetime(year=2022, month=5, day=1, hour=23))
    # The datetime at which you want staking to finish.
    EndTime.set(datetime.datetime(year=2023, month=1, day=31, hour=00))
...
def increaseDeposit(
    ...
    user = ctx.caller if user_ctx is "caller" else ctx.caller
    assert OpenForBusiness.get() == True, "This staking pool is not open right now."
