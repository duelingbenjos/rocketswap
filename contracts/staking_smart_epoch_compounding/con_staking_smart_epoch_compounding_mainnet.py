...
def seed():
    ...
    EpochMaxRatioIncrease.set(1000) # We're not incrementing the Epoch until the staking begins
    EpochMinTime.set(30 * 60 * 24 * 3)  # 3 days
    UseTimeRamp.set(True)
    ...
    Epochs[0] = {"time": now, "staked": 0, "amt_per_hr": 3424}
    ...
    EmissionRatePerHour.set(3424)
    DevRewardPct.set(1 / 10)

    # The datetime from which you want to allow staking.
    StartTime.set(datetime.datetime(year=2021, month=5, day=2, hour=18))
    # The datetime at which you want staking to finish.
    EndTime.set(datetime.datetime(year=2022, month=5, day=2, hour=0))
