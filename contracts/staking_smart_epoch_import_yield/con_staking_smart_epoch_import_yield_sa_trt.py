...
def seed():
    ...
    Epochs[0] = {"time": now, "staked": 0, "amt_per_hr": 3174} # 10000000 / (105 * 24)

    meta["version"] = "0.0.2"
    meta[
        "type"
    ] = "staking_smart_epoch"  # sta # staking || lp_farming || etcetera ...
    meta["STAKING_TOKEN"] = "con_trt"
    meta["YIELD_TOKEN"] = "con_trt"
    ...
    StartTime.set(datetime.datetime(year=2023, month=1, day=18, hour=22))
    # The datetime at which you want staking to finish.
    EndTime.set(datetime.datetime(year=2023, month=4, day=30, hour=22))
...
def calculateYield(deposit):
    ...
    amount = deposit.get("amount")# now - now // 0 delta
...
def recoverYieldToken():
    ...
    YIELD_TOKEN.transfer(amount=total_available, to=Owner.get())
