<script>
    ...
    $: showCompoundButton = shouldShowCompoundButton(yieldToken, stakingContractType, totalStaked)
    ...
    const shouldShowCompoundButton = (yieldToken, stakingContractType, totalStaked) => {
        if (!yieldToken || !stakingContractType) return false
        if (!stakingInfo.OpenForBusiness) return false
        if (totalStaked.isLessThanOrEqualTo(0)) return false
