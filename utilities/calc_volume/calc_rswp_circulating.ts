...
async function getBalances(): Promise<any> {
	const res: any = await axios.get(`http://78.141.225.103:3535/current/all/con_rswp_lst001/balances`);
	return res.data;
}
...
function getBalanceValue(account: any) {
	...
	if (self_value) {
		return typeof self_value === "number" ? self_value : self_value.__fixed__;
	} else {
		return self_value;
	}
}
